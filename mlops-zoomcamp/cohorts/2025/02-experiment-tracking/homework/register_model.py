...
def run_register_model(data_path: str, top_n: int):
    ...
    # Get top N runs from hyperopt experiment based on RMSE
    ...
    # Train and log each top model, track new run_ids
    run_ids = []
    for run in hpo_runs:
        run_id = train_and_log_model(data_path=data_path, params=run.data.params)
        run_ids.append(run_id)

    # Get the run with the lowest test RMSE in the "random-forest-best-models" experiment
    ...
    print(f"Model registered: {MODEL_NAME} from run_id: {best_run_id}")
